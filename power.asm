...
; Note that as far as the C Library is concerned, command line arguments are always strings. 
; If you want to treat them as integers, call atoi
; ----------------------------------------------------------------------------
...
_main:
	...
	mov	ebx, [esp+20]		; argv
	push	dword [ebx+4]		; argv[1]  ; start of first process / x
	call	_atoi
	add	esp, 4
	mov	esi, eax		; x in esi
	push	dword [ebx+8]
	call	_atoi			; argv[2]  ; second process / y
	add	esp, 4
	cmp	eax, 0
	jl	error2
